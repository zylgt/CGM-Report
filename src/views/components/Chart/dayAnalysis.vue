        ...
        <div class="chart-box">
          <div class="analysis-info">
            <div class="analysis-info-item">
              <div
                :class="[
                  dayInfo.avg >= 118.8 ? 'active' : '',
                  'analysis-info-value',
                ]"
                v-if="unit == 'mg/dL'"
              >
                {{ dayInfo.avg }}
                <span class="analysis-info-value-unit">mg/dL</span>
              </div>
              <div
                :class="[
                  dayInfo.avg >= 6.6 ? 'active' : '',
                  'analysis-info-value',
                ]"
                v-else
              >
      ...
      >
          ...
          <template slot-scope="scope">
            <div class="table-event-type">
              <img
                ...
                alt=""
                class="table-event-icon"
                v-if="scope.row.event_type == 0"
              />
              <img
                src="~@/assets/image/event-icon1.png"
                alt=""
                class="table-event-icon"
                v-if="scope.row.event_type == 1"
              />
              <img
                src="~@/assets/image/event-icon2.png"
                alt=""
                class="table-event-icon"
                v-if="scope.row.event_type == 3"
              />
              <img
                src="~@/assets/image/event-icon3.png"
                alt=""
                class="table-event-icon"
                v-if="scope.row.event_type == 2"
              />
              <img
                src="~@/assets/image/event-icon4.png"
                alt=""
                class="table-event-icon"
                v-if="scope.row.event_type == 4"
              />
              <img
                src="~@/assets/image/event-icon5.png"
                alt=""
                class="table-event-icon"
                v-if="scope.row.event_type == 5"
              />
              <img
                src="~@/assets/image/event-icon6.png"
                alt=""
                class="table-event-icon"
                v-if="scope.row.event_type == 6"
              />
              <img
                src="~@/assets/image/event-icon7.png"
                alt=""
                class="table-event-icon"
                v-if="scope.row.message_type == 'high'"
              />
              <img
                src="~@/assets/image/event-icon8.png"
                alt=""
                class="table-event-icon"
                v-if="scope.row.message_type == 'low'"
              />
              <img
                src="~@/assets/image/event-icon8.png"
                alt=""
                class="table-event-icon"
                v-if="scope.row.message_type == 'elow'"
              />
              <span v-if="scope.row.event_type == 0">{{
                scope.row.event_body.eat_name
              }}</span>
              <span v-if="scope.row.event_type == 1">{{
                scope.row.event_body.sport_name
              }}</span>
              <span v-if="scope.row.event_type == 2">{{
                scope.row.event_body.medicine_name
              }}</span>
              <span v-if="scope.row.event_type == 3">{{
                scope.row.event_body.insulin_name
              }}</span>
        ...
        <el-table-column prop="value" label="测量值">
          <template slot-scope="scope">
            ...
            <div v-if="scope.row.event_type == 5">
              {{ scope.row.event_body.bg }}{{ unit }}
            </div>
            <div v-if="scope.row.bgValue">
              {{ scope.row.bgValue }}{{ unit }}
            </div>
          </template>
        </el-table-column>
      </el-table>
      ...
      option: {
        ...
        series: [
          //中位线
          {
            ...
            showSymbol:
              _.compact(this.dataList.value).length > 1 ? false : true,
            symbolSize: 1,
            symbol: "circle",
            emphasis: {
              disabled: true,
              ...
              data: [
                {
                  coord: [
                    _.indexOf(
                      this.dataList.value,
                      _.compact(this.dataList.value)[0]
                    ),
                    _.compact(this.dataList.value)[0],
                  ],
                },
  ...
  methods: {
    ...
    handelOption(data) {
      ...
      if (dayInfo.resultValue.length > 0) {
        ...
        let filterArr = dayInfo.resultValue.filter(
          (val) => val >= 36 && val <= 540
        );
        ...
        dayInfo.lowTir = (
          Number(dayInfo.allTir.lowRate) + Number(dayInfo.allTir.veryLowRate)
        ).toFixed(1);
        ...
        if (unit != "mg/dL") {
          dayInfo.value = dayInfo.value.map((val) =>
            GlucoseUtils.mgdlToMmol(val)
          );
        }
        this.option.tooltip = {
          show: true,
          ...
          formatter(params) {
            let h =
              Math.floor(Number(params[0].axisValue) / 60) < 10
                ? "0" + Math.floor(Number(params[0].axisValue) / 60)
                : Math.floor(Number(params[0].axisValue) / 60);
            let m =
              Number(params[0].axisValue) % 60 < 10
                ? "0" + (Number(params[0].axisValue) % 60)
                : Number(params[0].axisValue) % 60;
            let moment = h + ":" + m;
            let html =
              "<div class='tooltip-box' >" +
              " <div class='tooltips-val'>" +
              " <span class='tooltips-val-num'>" +
              params[0].value +
              "</span>" +
              unit +
              "</div>" +
        ...
        this.option.series[0].markPoint.data = [
          {
            coord: [
              _.indexOf(dayInfo.value, _.compact(dayInfo.value)[0]),
              _.compact(dayInfo.value)[0],
            ],
          },
        ];
        this.option.series[0].markLine.data =
          this.option.series[0].markLine.data.splice(0, 2);
        this.option.xAxis[0].data = xData;
        this.option.yAxis[0].max =
          unit == "mg/dL" ? GlucoseUtils.mmolToMgdl(max) : max;
        this.option.yAxis[0].min = unit == "mg/dL" ? 36 : 2;
        this.option.yAxis[0].interval =
          unit == "mg/dL"
            ? (GlucoseUtils.mmolToMgdl(max) - 36) / 4
            : (max - 2) / 4;
