<template>
  <div>
    <div class="agp-chart-empty" v-if="empty">
      ...
      <div class="agp-chartempty-tips">
        At least 5 days of data are required.
      </div>
    </div>
    <div v-else>
...
export default {
  ...
  mounted() {
    ...
    if (
      _.compact(data.agp05).length > 1 &&
      _.compact(data.agp25).length > 1 &&
      _.compact(data.agp50).length > 1 &&
      _.compact(data.agp75).length > 1 &&
      _.compact(data.agp95).length > 1
    ) {
      this.renderIng(data);
      this.empty = false;
    } else {
    ...
    renderIng(data) {
      ...
      if (unit != "mg/dL") {
        ...
        this.option.series[1].data = value.agp25.map((item, index) => {
          return item
            ? GlucoseUtils.mgdlToMmol(item - value.agp05[index])
            : item;
        });
        this.option.series[2].data = value.agp75.map((item, index) => {
          return item
            ? GlucoseUtils.mgdlToMmol(item - value.agp25[index])
            : item;
        });
        this.option.series[3].data = value.agp95.map((item, index) => {
          return item
            ? GlucoseUtils.mgdlToMmol(item - value.agp75[index])
            : item;
        });
