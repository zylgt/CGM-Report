    ...
    <div class="tir-info">
      <div class="tir-key">
          ...
          >
            <div class="tir-key-box">
              ...
              <div class="tir-key-desc">
                ...
                <div class="tir-key-range">
                  {{ unit == "mg/dL" ? item.descmg : item.desc }}
                </div>
              </div>
            </div>
            <div class="tir-value">
              <div class="tir-key-key">
                <span
                  v-if="item.index == 2"
                  :class="[
                    Number(item.value) <= item.target ? 'active' : '',
                    'tir-print-keys',
                  ]"
                  >{{ item.value }}%</span
                >
              ...
              <div class="tir-key-range" v-if="item.index == 2">
                Target＞{{ item.target }}%
              </div>
              <div
                class="tir-key-range"
                v-if="item.index == 0 || item.index == 4"
              >
                Target＜{{ item.target }}%
              </div>
            </div>
        ...
        <div
          class="tir-all-2"
          v-if="lowTarget != 0"
          :style="{ height: lowTargetHeight + 'px' }"
        >
          <div :class="[lowLine ? '' : 'disabled', 'tir-all-line']"></div>
          <div class="tir-all-value">
            <div
              :class="[Number(lowTarget) >= 4 ? 'active' : '', 'tir-print-key']"
            >
              {{ lowTarget }}%
            </div>
...
export default {
  ...
  methods: {
    ...
    renderData(data) {
      ...
      this.levels[0].duration = [
        Math.floor(result.veryHighDuration / 60),
        result.veryHighDuration % 60,
      ];
      this.levels[1].duration = [
        Math.floor(result.highDuration / 60),
        result.highDuration % 60,
      ];
      this.levels[2].duration = [
        Math.floor(result.normalDuration / 60),
        result.normalDuration % 60,
      ];
      this.levels[3].duration = [
        Math.floor(result.lowDuration / 60),
        result.lowDuration % 60,
      ];
      this.levels[4].duration = [
        Math.floor(result.veryLowDuration / 60),
        result.veryLowDuration % 60,
      ];

      this.levels[2].desc = result.target
        ? result.target[0] + "-" + result.target[1] + "mmol/L"
        : "";
      this.levels[2].descmg = result.target
        ? result.target[0] + "-" + result.target[1] + "mg/dL"
        : "";
      this.levels[1].desc = result.target
        ? _.round(result.target[1] + 0.1, 1) + "-13.9mmol/L"
        : "";
      this.levels[1].descmg = result.target
        ? result.target[1] + 1 + "-250mg/dL"
        : "";
      this.levels[3].desc = result.target
        ? "3.0-" + _.round(result.target[0] - 0.1, 1) + "mmol/L"
        : "";
      this.levels[3].descmg = result.target
        ? "54-" + (result.target[0] - 1) + "mg/dL"
        : "";
      this.highTarget =
        Number(this.levels[0].value) || Number(this.levels[1].value)
          ? _.round(
              Number(this.levels[0].value) + Number(this.levels[1].value),
              1
            )
          : 0;
      this.lowTarget =
        Number(this.levels[3].value) || Number(this.levels[4].value)
          ? _.round(
              Number(this.levels[3].value) + Number(this.levels[4].value),
              1
            )
    ...
    handelHeight() {
      ...
      if (this.lowLine) {
        // 计算高度
        this.lowTargetHeight =
          legend[legend.length - 1].offsetTop -
          legend[legend.length - 2].offsetTop;
    ...
    drawTir() {
      ...
      for (var i = 0; i < this.tirList.length; i++) {
        ...
        if (tir_key_top - start_y > 10 && start_y + value - tir_key_top > 10) {
          ...
          noRangeLength++;
          lineOneX =
            tir_key_top < start_y + value
              ? lineStart + lineWidth - (lineWidth - noRangeLength * 3)
              : lineStart + (lineWidth - noRangeLength * 3) - lineWidth / 2;
